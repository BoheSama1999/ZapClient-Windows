...
struct ConfigManager
{
	...
	void LoadDefaultConfig() {
		std::ifstream Default("settings.txt");
		if (!Default.is_open()) {
		...
		if (strcpy_s(configName, sizeof(configName), DefaultConfigName.c_str()) != 0) {
			std::cerr << "Failed to copy config name" << std::endl;
			return;
		}
		...
		ImGui::BeginListBox("##Configs", ImVec2(600, 300));
		{
			...
			for (auto config : configFiles) {
				...
				if (ImGui::Selectable(config.c_str(), is_selected)) {
					selectedConfig = n;
					strcpy_s(configName, sizeof(configName), config.c_str()); 
				}
